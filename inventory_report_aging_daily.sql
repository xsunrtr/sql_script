...
select 
count(*),
count(CASE WHEN day(time_to_next_scan)>=2 AND day(time_to_next_scan) < 3 THEN 1 END) AS '2-3 days',
count(CASE WHEN day(time_to_next_scan)>=3 AND day(time_to_next_scan)<10 THEN 1 END) AS '3-10 days',
count(CASE WHEN day(time_to_next_scan)>=10 AND day(time_to_next_scan)<30 THEN 1 END) AS '10-30 days',
count(CASE WHEN day(time_to_next_scan)>=30 AND day(time_to_next_scan)<180 THEN 1 END) AS '30-180 days',
count(CASE WHEN day(time_to_next_scan)>=180 THEN 1 END) AS '180 days and above'
