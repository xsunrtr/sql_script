...
, apr."type"
, apr.sub_type
, case when po_category is not null then apr.group_id else null end as PO_group_id
, case when po_category is not null then apr.reservation_booking_id else null end as PO_reservation_booking_id
, case when apr.po_category is not null and ogre.child_sku is null
	then apr.reservation_booking_id else null end as Reservations_ParentSku_PO_booking_id
, case when apr.po_category is not null and ogre.child_sku is not null
	then apr.reservation_booking_id else null end as Reservations_Extras_PO_booking_id
...
left join order_group_reservation_withExtras as ogre
...
,apr.sub_type
,apr.po_category
--- Standardize PO reason and further breakdown deactivation reason as po reason
, CASE WHEN lower(apr.po_reason) like '%late%' then 'Lateness'  
     	 ...
     	 WHEN apr.po_category ='Predicated' then apr.po_reason
     	 WHEN apr.po_category = 'Warehouse' and lower(apr.po_reason) like '%miss%' then 'Missing'
     	 WHEN apr.po_reason = 'DEACTIVATION' 
     	 and apr.deactivation_reason = 'Auto - Late Return' 
     	 or apr.deactivation_reason ='Lost/Stolen-Customer' then 'Deactivation - Lost/Stolen'  	 
...
, case when po_category is not null then ogrw.reservation_booking_id else null end as PO_reservation_booking_id
...
, case when ups.po_category is not null and ogrw.child_sku is null
	then ogrw.reservation_booking_id else null end as Reservations_ParentSku_PO_booking_id
, case when ups.po_category is not null and ogrw.child_sku is not null
	then ogrw.reservation_booking_id else null end as Reservations_Extras_PO_booking_id
 from 
  ( select  fulfillment_id
			...
			, 1 as cancel_flag
    from    rtrbi.order_group_reservation_canceled_unlimited_temp as ogrq
...
left join unlimited_po_sources as ups 
 on ogrw.reservation_booking_id = ups.booking_id
...
,count(distinct case when ups.po_category is not null and ogrw.child_sku is null
	then ogrw.reservation_booking_id else null end) as Reservations_ParentSku_PO_count
...
,count(distinct case when ups.po_category is not null 
 ...
 from order_group_reservation_unlimited_include_canceled As ogrw
left join unlimited_po_sources as ups 
 on ogrw.reservation_booking_id = ups.booking_id
